...
const GalleryManager = () => {
  ...
  const fetchItems = async (page = 1) => {
    ...
    setIsLoading(true);
    try {
      const res = await axios.get(`${apiUrl}/memeber/galleries`, {
    ...
    try {
      ...
      if (editingItem) {
        ...
        res = await axios.post(
          `${apiUrl}/memeber/galleries/${editingItem.id}`,
          formData,
          {
            headers: {
      ...
      // Now res is guaranteed to exist if the request succeeded
      if (res.data?.status) {
        toast.success(editingItem ? 'Updated successfully' : 'Added successfully');
        fetchItems(1); // refresh from page 1 to see newest items
        closeModal();
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await axios.delete(
        `${apiUrl}/memeber/galleries/${confirmDeleteId}`,
        {
          headers: { Authorization: `Bearer ${user?.token}` },
        }
      );
  ...
  const handleToggleActive = async (id, isCurrentlyActive) => {
    ...
    try {
      const res = await axios.patch(
        `${apiUrl}/memeber/galleries/${id}/toggle-active`,
        {},
        {
          headers: { Authorization: `Bearer ${user?.token}` },
        }
  ...
  <div className="max-h-[60vh] overflow-auto">   {/* vertical scroll if many rows */}
                ...
                <div className="p-6 md:p-8">
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    <div>
                      ...
                      />
                      {errors.image && (
                        <p className="mt-2 text-sm text-red-600">Image is required for new items</p>
                      )}
                    </div>
