...
import { apiUrl } from '../https'; // adjust path
import { AdminAuthContext } from '../../context/AdminAuth'; // adjust path
...
const VideoManager = () => {
  ...
  const openAddModal = () => {
    setEditingItem(null);
    reset({ thumbnail: null, video_url: '' });
    setIsModalOpen(true);
  };
    ...
    try {
      ...
      if (editingItem) {
        ...
        res = await axios.post(
          `${apiUrl}/memeber/videos/${editingItem.id}`,
          formData,
          {
            headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await axios.delete(
        `${apiUrl}/memeber/videos/${confirmDeleteId}`,
        {
          headers: { Authorization: `Bearer ${user?.token}` },
        }
      );
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 p-6 md:p-8">
        ...
        <div className="flex flex-col items-start justify-between gap-4 mb-8 sm:flex-row sm:items-center">
          <h1 className="text-2xl font-bold text-gray-900 md:text-3xl">Video Management</h1>
          <motion.button
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  {/* <th className="px-6 py-4 text-sm font-semibold text-left text-gray-700">Thumbnail</th> */}
                  <th className="px-6 py-4 text-sm font-semibold text-left text-gray-700">Video URL</th>
                  ...
                  items.map((item) => (
                    <tr key={item.id} className="transition-colors hover:bg-gray-50">
                      ...
                      <td className="max-w-md px-6 py-5 text-sm text-gray-600 truncate">
                        {item.video_url || <span className="italic text-gray-400">No URL</span>}
                      </td>
                      <td className="px-6 py-5">
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    {/* <div>
                      ...
                      <input
                        type="file"
                        accept="image/*"
                        // {...register('thumbnail', { required: !editingItem })}
                      ...
                      />
                      {errors.thumbnail && (
                        <p className="mt-2 text-sm text-red-600">Thumbnail is required</p>
                      )}
                    </div> */}
                    ...
                    <div>
                      ...
                      />
                      {errors.video_url && (
                        <p className="mt-2 text-sm text-red-600">Video URL is required</p>
                      )}
                    </div>

                    {editingItem?.thumbnail && (
                      <div className="mt-4">
                        <p className="mb-2 text-sm font-medium text-gray-700">Current Thumbnail:</p>
                        <img
                          src={getImageUrl(editingItem.thumbnail)}
