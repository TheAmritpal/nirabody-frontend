// backend/pages/websitechange/BlogCategoryManager.jsx
...
const BlogCategoryManager = () => {
  ...
  const fetchItems = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`${apiUrl}/memeber/blog-categories`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingItem) {
        res = await axios.put(
          `${apiUrl}/memeber/blog-categories/${editingItem.id}`,
        ...
        res = await axios.post(
          `${apiUrl}/memeber/blog-categories`,
          { name: categoryName, icon: categoryIcon },
          {
            headers: {
              'Content-Type': 'application/json',
  ...
  const toggleActive = async (id) => {
    try {
      const res = await axios.post(
        `${apiUrl}/memeber/blog-categories/${id}/toggle`,
        {},
        {
          headers: { Authorization: `Bearer ${user?.token}` },
        }
  ...
  const handleDelete = async () => {
    ...
    setActionLoading(true);
    try {
      await axios.delete(`${apiUrl}/memeber/blog-categories/${confirmDeleteId}`, {
        headers: { Authorization: `Bearer ${user?.token}` },
      });
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan={6} className="text-center py-12"><div className="animate-spin w-8 h-8 border-4 border-indigo-500 rounded-full border-t-transparent mx-auto"></div></td></tr>
                ) : items.length === 0 ? (
                  <tr><td colSpan={6} className="text-center py-12 text-gray-500">No categories found</td></tr>
                ) : (
              ...
              <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={categoryName} onChange={(e) => setCategoryName(e.target.value)} placeholder="Enter category name" required className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Icon (Font Awesome class)</label>
                    <input type="text" value={categoryIcon} onChange={(e) => setCategoryIcon(e.target.value)} placeholder="fas fa-tag" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" />
                    <p className="text-xs text-gray-500 mt-1">Example: fas fa-spa, fas fa-leaf</p>
                  </div>
